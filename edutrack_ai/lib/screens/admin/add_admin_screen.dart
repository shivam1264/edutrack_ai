...
class _AddAdminScreenState extends State<AddAdminScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Register Admin', style: TextStyle(fontWeight: FontWeight.w900)),
        backgroundColor: Colors.white,
            ...
            children: [
              Center(
                child: Container(
                  ...
                  child: const Icon(Icons.admin_panel_settings_rounded, color: Colors.redAccent, size: 40),
                ),
              ),
              const SizedBox(height: 32),
              _buildLabel('Admin Full Name'),
              _buildField(_nameCtrl, 'Enter full name', Icons.person_outline_rounded),
              const SizedBox(height: 20),
              _buildLabel('Official Email (Login ID)'),
  ...
  Widget _buildSubmitButton() {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text('Onboard Admin', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
      ),
    );
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Super Admin account registered successfully! 🛡️'), backgroundColor: Colors.green));
      }
    } catch (e) {
